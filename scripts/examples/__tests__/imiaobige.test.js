...
import s from "../sources/imiaobige";
...
test("search", async () => {
  ...
  expect(value[2]).toStrictEqual({
    id: "113672",
    title: "生活系游戏",
    url: "https://www.imiaobige.com/novel/113672.html",
    author: "吨吨吨吨吨",
    cover: "https://img.imiaobige.com/113672/252500.jpg",
    intro: `【一份卖相不好的宫保鸡丁】
从江枫无意中发现自己居然可以看到自家老爹炒出来的菜的备注开始，他的人生就已经发生了翻天覆地的变化
……
1.本游戏自由度极高，请玩家自行探索.
...
test("fetch chapters", async () => {
  const source = new BookSource(s, null);
  const result = await source.chapters("113672");
  source.destroy();
